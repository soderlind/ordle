...
export default function Game() {
  ...
  const onInput = (event) =>
    ...
    setGuesses((oldGuesses) => [
      ...
      event.target.value.replace(/[^a-zæøå]/gi, "").toLowerCase(),
    ]);
  ...
  const onSubmit = (event) => {
    ...
    } else setMessage(`"${guess}" finnes ikke i ordlisten`);
  };
  ...
  const onMessageClick = () => {
    if (isCorrect) {
      setMessage("Kopiert og kan deles (ctrl-v)! 📋");

      const clipboardMessage = `Ordle ${answerIndex} ${
        guesses.length - 1
      }/${MAX_GUESSES}
